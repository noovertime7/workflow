...
import {MysqlExport} from "./node/mysql_export";
import {MysqlImport} from "./node/mysql_import";
import {PostgreSql} from "./node/postgre";
import {Notifications} from "./node/dingding";
import {ApproveDeploy} from "./node/ApproveDeploy";
import {DelayTime} from "./node/delay_time";
export class CustomX6NodeProxy {
  ...
  getData(graph?: Graph, workflowData?: IWorkflow): IWorkflowNode {
    ...
    switch (obj.type) {
      ...
      case NodeTypeEnum.WEBHOOK:
        ...
        break;
      case NodeTypeEnum.MysqlExport:
        nodeData = MysqlExport.build(obj);
        break;
      case NodeTypeEnum.MysqlImport:
        nodeData = MysqlImport.build(obj);
        break;
      case NodeTypeEnum.PostgreSql:
        nodeData = PostgreSql.build(obj);
        break;
      case NodeTypeEnum.ApproveDeploy:
        nodeData = ApproveDeploy.build(obj);
        break;
      case NodeTypeEnum.Notifications:
        nodeData = Notifications.build(obj);
        break;
      case NodeTypeEnum.DelayTime:
        nodeData = DelayTime.build(obj);
        break;
  ...
  setData(data: IWorkflowNode): void {
    ...
    console.log("node保存数据",this.node)
