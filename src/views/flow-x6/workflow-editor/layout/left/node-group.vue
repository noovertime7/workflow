  ...
  <div class="group" v-if="isShow">
    ...
    <div class="nodes" v-show="!collapsed">
      ...
      <template v-else-if="nodes.length>0">
        ...
        <!-- 显示更多 -->
        <!--        <div class="load-more">-->
        <!--          <el-load-more :state="loadState" :load-more="btnDown"></el-load-more>-->
        <!--        </div>-->
      </template>
    </div>
  </div>
...
export default defineComponent({
  ...
  setup(props, {emit}) {
    const groupName = computed<string>(() => {
      if (props.type === NodeGroupEnum.Devops) {
        return '自动部署组件';
      } else if (props.type === NodeGroupEnum.Database) {
        return '数据库操作';
      } else {
        return '';
      }
    ...
    // 组件初始加载的节点个数
    const initialPageSize: number = 9999;
    ...
    const loadNodes = async (keyword: string, reload: boolean, pageSize: number, currentPage: number = 1) => {
      ...
      try {
        ...
        switch (props.type) {
            // 触发器
          case NodeGroupEnum.Devops:
            nodes.value = workflowNode.loadDevops(keyword);
            break;
            // 内置节点
          case NodeGroupEnum.Database:
            nodes.value = workflowNode.loadDatabaseNodes(keyword);
            break;
            // 本地节点
