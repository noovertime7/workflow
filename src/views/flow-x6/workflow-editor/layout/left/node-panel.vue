<template>
  <div :class="{ 'jm-workflow-editor-node-panel': true, collapsed }" ref="container">
    ...
    <div class="search">
      <el-input placeholder="回车搜索" v-model="keyword" @change="changeKeyword" :clearable="true">
        <template #prefix>
    ...
    <el-scrollbar>
      <div class="groups" v-show="nodeCount>0">
        <node-group class="trigger" type="devops" :keyword="tempKeyword" @get-node-count="getNodeCount"/>
        <node-group class="inner" type="database"   :keyword="tempKeyword" @get-node-count="getNodeCount"/>
<!--        <node-group type="local"   :keyword="tempKeyword" @get-node-count="getNodeCount"/>-->
<!--        <node-group type="official"   :keyword="tempKeyword" @get-node-count="getNodeCount"/>-->
<!--        <node-group type="community"   :keyword="tempKeyword" @get-node-count="getNodeCount"/>-->
      </div>
      <div class="empty" v-if="nodeCount<=0">
        <el-empty description="没有搜到相关结果" >
        </el-empty>
...
export default defineComponent({
  components: {ArrowLeft, Search, NodeGroup },
  emits: ['node-selected'],
  setup(props, { emit }) {
