...
onMounted(() => {
  destroyFn.value = initGraph(proxy)
})
...
const initGraph = (proxy:any) => {
  const graph = FlowGraph.init(proxy ,$('#container'), $('#container').getBoundingClientRect().width, $('#container').getBoundingClientRect().height)
  isReady.value = true
  const resizeFn = () => {
